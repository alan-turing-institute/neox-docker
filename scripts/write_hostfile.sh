#!/bin/bash
GPUS_PER_NODE=4
# save dynamically generated hostfile to local tmp storage (does not presist after job end)
mkdir -p /tmp/hostfiles/hosts_$SLURM_JOBID
# need to add the current slurm jobid to hostfile name so that we don't add to previous hostfile
hostfile=/tmp/hostfiles/hosts_$SLURM_JOBID/for_deepspeed.txt
